...
public class BulkDealsFilter extends AbstractDBFilter<Equity> implements EquityFilter {
    ...
    public Criteria getCriteria(String parentObject) {
        ...
        if(getMaxSales()!=null){
            criteriaList.add(Criteria.where(parentObject+".bulkDealDetails.saleDeals."+(this.getMaxSales())).exists(false));
        }
        if(getMinPurchases()!=null){
            criteriaList.add(Criteria.where(parentObject+".bulkDealDetails.purchaseDeals."+(this.getMinPurchases()-1)).exists(true));
        }
